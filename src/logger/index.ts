...
class Logger {
  ...
  info(message, title,) {
    if (this.enabled) {
      const titleFormatted = colors.formatTitle('info', title || 'Info');
  ...
  note(message, title,) {
    if (this.enabled) {
      const titleFormatted = colors.formatTitle('note', title || 'Note');
      this.log(titleFormatted, message);
    }
  }
  ...
  error(message, title,) {
    if (this.enabled) {
      const titleFormatted = colors.formatTitle('error', title || 'Error');
      this.log(titleFormatted, message);
    }
  }
