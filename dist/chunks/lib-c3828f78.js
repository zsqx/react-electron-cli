...
var getConfig = require('./lib-1802974f.js');
...
const { ELECTRON_MAIN_ENTRY, ELECTRON_MAIN_OUTPUT: ELECTRON_MAIN_OUTPUT$1, mainWebpack } = getConfig.getConfig();
...
const config$2 = () => ({
    ...
    output: {
        ...
        filename: '[name].js',
        path: path.resolve(ELECTRON_MAIN_OUTPUT$1)
    },
...
var config$3 = () => webpackMerge.mergeWithCustomize({
    customizeArray: webpackMerge.unique('plugins', ['HtmlWebpackPlugin'], plugin => plugin.constructor && plugin.constructor.name)
})(config$2(), mainWebpack);
...
class Logger {
    ...
    info(message, title) {
        if (this.enabled) {
            const titleFormatted = formatTitle('info', title || 'Info');
            this.log(titleFormatted, message);
        }
    }
    note(message, title) {
        if (this.enabled) {
            const titleFormatted = formatTitle('note', title || 'Note');
            this.log(titleFormatted, message);
        }
    }
    error(message, title) {
        if (this.enabled) {
            const titleFormatted = formatTitle('error', title || 'Error');
            this.log(titleFormatted, message);
        }
    }
...
const startElectron = function () {
    const complier = webpack(webpackMerge.merge(config$3(), {
        // @ts-ignore
        mode: 'development',
        plugins: []
    }));
...
var startElectron$1 = startElectron;

const processCwd = process.cwd();
const { ELECTRON_RENDERER_OUTPUT: ELECTRON_RENDERER_OUTPUT$2, ELECTRON_RENDERER_ENTRY, renderWebpack } = getConfig.getConfig();
/** @type {import('webpack').Configuration} */
const getWebpackConfig = () => {
    ...
    return {
        ...
        output: {
            ...
            path: ELECTRON_RENDERER_OUTPUT$2,
            clean: true,
...
var config$1 = () => webpackMerge.mergeWithCustomize({
    customizeArray: webpackMerge.unique('plugins', ['HtmlWebpackPlugin'], plugin => plugin.constructor && plugin.constructor.name)
})(getWebpackConfig(), renderWebpack);

const { PORT, ELECTRON_RENDERER_OUTPUT: ELECTRON_RENDERER_OUTPUT$1 } = getConfig.getConfig();
// @ts-ignore
const startRender = (onlyWeb = true) => {
    process.env.RENDERER_PATH = `http://localhost:${PORT}`;
    const devConfig = webpackMerge.merge(config$1(), {
        mode: 'development',
        devServer: {
            port: PORT,
            host: '0.0.0.0',
            hot: true,
            compress: true,
            static: ELECTRON_RENDERER_OUTPUT$1,
...
let mainProcess;
const startApp = () => {
    mainProcess = node_child_process.spawn(electron, ['.', '--inspect']);
...
const buildMain$ = () => {
    return new rxjs.Observable(observer => {
        webpack(webpackMerge.merge(config$3(), {
            mode: 'production',
        }), (err, stats) => {
...
var buildMain$$1 = buildMain$;

const { ELECTRON_MAIN_OUTPUT, ELECTRON_RENDERER_OUTPUT } = getConfig.getConfig();
const buildWeb$ = () => {
    const { MODE, PLATFORM } = process.env;
    console.log(MODE, PLATFORM);
    return new rxjs.Observable(observer => {
        webpack(webpackMerge.merge(config$1(), {
            mode: 'production',
            output: {
...
var buildWeb$$1 = buildWeb$;

const { spawn } = require("cross-spawn");
const buildOptions = require(path.join(process.cwd(), 'package.json')).build;
// 剔除afterSign
if (buildOptions === null || buildOptions === void 0 ? void 0 : buildOptions.afterSign) {
    Reflect.deleteProperty(buildOptions, 'afterSign');
    Reflect.defineProperty(buildOptions, 'afterSign', {
        value: path.join(__dirname, './notarize.ts'),
        enumerable: true
    });
}
const buildElectron$ = (target) => {
    // 默认打包两个平台，暂时不考虑Linux
    const map = {
        'mac': '--mac',
        'win': '--win'
    ...
    const cp = spawn('electron-builder', Object.values(map), {
        stdio: 'inherit'
    });
    return rxjs.fromEvent(cp, 'close');
};
var buildElectron$$1 = buildElectron$;

const buildApp = (target) => {
    buildWeb$$1()
        .pipe(rxjs.tap(() => logger.title('info', 'Main进程', `开始打包`)), rxjs.mergeMap(() => buildMain$$1()), rxjs.tap(() => logger.title('success', 'Main进程', `Done`)), rxjs.mergeMap(() => buildElectron$$1(target)))
        .subscribe(() => logger.title('success', 'App', `Done`));
};
const buildWeb = () => {
    buildWeb$$1().subscribe();
};
